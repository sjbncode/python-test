...
def read_max_page(page):  # 获取当前招聘关键词的最大页数，大于30的将会被覆盖，所以最多只能抓取30页的招聘信息
    page_json = json.loads(page)    
    #print(page_json['content'])
    max_page_num=1
    #max_page_num = page_json['content'].get('totalPageCount')
    if max_page_num > 30:
        max_page_num = 30
...
def save_excel(fin_result, tag_name, file_name):  # 将抓取到的招聘信息存储到excel当中
    book = xlsxwriter.Workbook(r'd:\%s.xls' % file_name)  # 默认存储在桌面上
    #book = xlsxwriter.Workbook('d:\xx.xls')  # 默认存储在桌面上
